...
import berries.servermod.tcm.mixin.InitAccessor;
import berries.servermod.tcm.mixin.MainAccessor;
import berries.servermod.tcm.mixin.SidingAccessor;
...
public class RequestStopsDataC2SPacket extends PacketHandler {
    ...
    public void runServer(@NotNull MinecraftServer minecraftServer, ServerPlayerEntity serverPlayerEntity) {
        Main tscInstance = InitAccessor.getMain();
        World playerWorld = serverPlayerEntity.getEntityWorld();
        String tscDimensionId = Init.getWorldId(playerWorld);

        ObjectImmutableList<Simulator> simulators = ((MainAccessor)tscInstance).getSimulators();
        for(Simulator simulator : simulators) {
            if(simulator.dimension.equals(tscDimensionId)) {
                simulator.run(() -> {
                    boolean debugLogging = false;
                    if(debugLogging) {
                        TCM.LOGGER.info("RequestStopsDataC2SPacket: Start compiling stops data in " + tscDimensionId);
                    }
                    ...
                    try {
                        ...
                        if(siding != null) {
                            ...
                            if(depot == null) {
                                if(debugLogging) {
                                    TCM.LOGGER.warn("RequestStopsDataC2SPacket: Failed to find depot from siding!");
                                }
                                return;
                            }

                            boolean vehicleFound = false;
                            ObjectArraySet<Vehicle> vehicles = ((SidingAccessor)(Object)siding).getVehicles();

                            if (vehicles == null || vehicles.isEmpty()) {
                                TCM.LOGGER.warn("RequestStopsDataC2SPacket: Failed to find any vehicles from siding!");
                            }
                            ...
                            for(Vehicle vehicle : vehicles) {
                                if(vehicle.getId() == vehicleId) {
                                    ...
                                    if(debugLogging) {
                                        TCM.LOGGER.info("RequestStopsDataC2SPacket: Submitting Response Packet");
                                    }
                                    minecraftServer.submit(() -> {
                                        Init.REGISTRY.sendPacketToClient(serverPlayerEntity, new StopsDataS2CPacket(vehicleId, sidingId, simplifiedStopsData));
                                        if(debugLogging) {
                                            TCM.LOGGER.info("RequestStopsDataC2SPacket: Request Packet Sent to {}, Goodbye!", serverPlayerEntity.getGameProfile().getName());
                                        }
                                    });
                                    break;
                                }
                            ...
                            if(!vehicleFound) {
                                TCM.LOGGER.warn("RequestStopsDataC2SPacket: Failed to find vehicle from siding!");
                            }
                        } else {
                            TCM.LOGGER.warn("RequestStopsDataC2SPacket: Failed to find requested siding!");
                        }
                    } catch (Exception e) {
                        TCM.LOGGER.error("RequestStopsDataC2SPacket failed!", e);
                    }
                });
                break;
            }
