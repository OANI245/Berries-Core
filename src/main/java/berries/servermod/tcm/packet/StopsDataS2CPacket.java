...
public class StopsDataS2CPacket extends PacketHandler {
    ...
    public StopsDataS2CPacket(PacketBufferReceiver packetBufferReceiver) {
        ...
        for(int i = 0; i < routeAmount; i++) {
            ...
            for(int j = 0; j < routeStopAmount; j++) {
                ...
                boolean turnbackPlatform = packetBufferReceiver.readBoolean();
                VehicleDataCache.stopsDataByteCounter += 1; // 1 byte for boolean
                VehicleDataCache.SimplifiedStop simplifiedStop = new VehicleDataCache.SimplifiedStop(destination, customDestination, stationId, platformId, distance, turnbackPlatform);
    ...
    public void write(PacketBufferSender packetBufferSender) {
        ...
        for(VehicleDataCache.RouteStopsData routeStopsData : simplifiedStopsData.routeStopsData) {
            ...
            for(VehicleDataCache.SimplifiedStop stop : routeStopsData.stops) {
                ...
                packetBufferSender.writeBoolean(stop.turnbackPlatform);
            }
        }
    }
    ...
    public void runClient() {
        ...
        InitClient.REGISTRY_CLIENT.sendPacketToServer(new RequestMTRDataC2SPacket(stationIds, sidingIds, routeIds, platformIds));
    }
}
