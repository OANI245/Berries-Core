...
const leftPos3tc = getCubeVertices([0.846,2.072,0.688], [0.658,2.072,-0.688],[0.752,2.072,0],0,-180,-50);
const rightPos3tc = getCubeVertices([-0.846,2.072,-0.688],[-0.658,2.072,0.688], [-0.752,2.072,0],0,180,50);
...
function render(ctx, state, train) {
    ...
    if(state.RateLimitTc.shouldUpdate()) {
        ...
        let g = state.dhlcdtc.graphicsFor("lcd_door_left");
        drawScreen(g, state.routeInfo, state.trainStatus == STATUS_RETURNING_TO_DEPOT ? 1 : train.getThisRoutePlatformsNextIndex(), state.trainStatus,state.Ratex,train,1,state,ctx);//如果指示方向相反请尝试更改倒数第二个为0

        g = state.dhlcdtc.graphicsFor("lcd_door_right");
        drawScreen(g, state.routeInfo, state.trainStatus == STATUS_RETURNING_TO_DEPOT ? 1 : train.getThisRoutePlatformsNextIndex(), state.trainStatus,state.Ratex,train,0,state,ctx);//如果指示方向相反请尝试更改倒数第二个为1

        state.dhlcdtc.upload();
        ctx.setDebugInfo("tex",state.dhlcdtc.texture);
    }
}
