...
import berries.servermod.tcm.client.mixin.NativeImageAccessor;
...
public class GraphicsTexture implements Closeable {
    ...
    public GraphicsTexture(int width, int height) {
        ...
        identifier = new Identifier(UFEInfo.MOD_ID, String.format("dynamic/graphics/%s", UUID.randomUUID()));
        MinecraftClient.getInstance().execute(() -> {
            MinecraftClient.getInstance().getTextureManager().registerTexture(identifier,
                    new AbstractTexture(dynamicTexture.data));
        });
    ...
    public void upload(int dstOffsetX, int dstOffsetY, int srcOffsetX, int srcOffsetY, int width, int height) {
        if (srcOffsetX + width > this.width) {
            throw new IllegalArgumentException(
                    "offsetX + width should not be larger than the total image size! Have you subtracted width from offset?");
        }
        if (srcOffsetY + height > this.height) {
            throw new IllegalArgumentException(
                    "offsetY + height should not be larger than the total image size! Have you subtracted height from offset?");
        }
        ...
        RenderSystem.recordRenderCall(() -> {
            ...
            if (nativeImage != null) {
                dynamicTexture.bindTexture();
                nativeImage.upload(0, dstOffsetX, dstOffsetY, srcOffsetX, srcOffsetY, width, height, false, false,
                        false, false);
            }
        });
    }
    ...
    private void copyBuffer(int x, int y, int width, int height) {
        ...
        long nativeImagePointer = ((NativeImageAccessor) (Object) Objects
                .requireNonNull(dynamicTexture.getImage()).data).getPixels();
